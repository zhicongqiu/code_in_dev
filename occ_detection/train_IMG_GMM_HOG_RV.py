        ...
        for k in range(1,resize_x/5+1):
            for kk in range(1,resize_y/5+1):
                ...
                gray_image = cv2.cvtColor(v_resized[5*(kk-1):5*kk,5*(k-1):5*k],
                                          cv2.COLOR_BGR2GRAY)
                if GMM_patch == []:
                    GMM_patch = gray_image.copy()
                else:
                    GMM_patch = np.vstack((GMM_patch,gray_image))
    ...
    for v_img in range(len(Vh[v_num][0])):
        ...
        for k in range(1,resize_x/5+1):
            for kk in range(1,resize_y/5+1):
                ...
                #turn into grey
                gray_image = cv2.cvtColor(re_img[5*(kk-1):5*kk,5*(k-1):5*k],
                                          cv2.COLOR_BGR2GRAY)
                for prototype in v_num_GMM:
                    temp = v_num_GMM[prototype].score(gray_image)
                    if temp>max_score:
                        max_score = temp
